...
from seq2seq.beam_lambda import BeamSearch, BeamSearchNode  # the new beam search alternative
...
def get_args():
    ...
    # default is 0.0 which means no regularization
        ...
        for _ in range(args.max_len-1):
            ...
            for i in range(log_probs.shape[0]):
                for j in range(args.beam_size):
                    ...
                    if next_word[-1] == tgt_dict.eos_idx:  # if the last word is EOS
                        node = BeamSearchNode(
                            search, node.emb, node.lstm_out, node.final_hidden,
                            node.final_cell, node.mask, torch.cat((prev_words[i][0].view([1]),
                            next_word)), node.logp, node.length, node.logp_sq
                            )
                        search.add_final(-node.eval(args.alpha, args.lambdaval), node)  # add the node to the list of completed sentences
                    ...
                    # Add the node to current nodes for next iteration
                        ...
                        search.add(-node.eval(args.alpha, args.lambdaval), node)  # add the node to the list of current nodes
